...
//             res.json({
//                 message: "le role a bien Ã©tÃ© crÃ©e",
//                 tag: addRole,
//             });
//             adminLog.log('info', {                                          // Log l'action de crÃ©ation d'un role
//                 url: req.url,
//                 method: req.method,
//                 user: `${req.userProfil[0].firstname} - ${req.userProfil[0].email}`,
//                 role: req.user.name,
//                 message: `CrÃ©ation du role ${req.body.name}`
//             });
//         } else {
//             res.status(409).json({
//                 error: "Le nom du rÃ´le existe dÃ©jÃ  !",
//             });
//         }
//     } catch(error) {
//         res.status(500).json(error.message);
...
//         if(roleExist) {
//             const getRole = await Role.findByPk(req.params.id);                   // RÃ©cupÃ¨re les infos du role qui va Ãªtre modifiÃ©
//             req.body.name = (req.body.name).toLowerCase();
...
//                 res.json({
//                     message: "le role a bien Ã©tÃ© mise Ã  jour",
//                     role: role,
//                 });
...
//             } else {
//                 res.status(409).json({
//                     error: "Le nom du role existe dÃ©jÃ ",
//                 });
//             }
//         } else {
//             res.status(404).json({
...
//     try {
//         const roleExist = await Role.checkExist(req.params.id);         // Controle si l'id du role existe
//         const getRole = await Role.findByPk(req.params.id);             // RÃ©cupÃ¨re les infos du role  Ã  supprimer
//         if(roleExist) {
//             const role = await Role.delete(req.params.id);
//             res.json({
//                 message: "le tag a bien Ã©tÃ© supprimÃ©",
//                 role: role,
//             });
